...
#### 4.5.1 量级与符号：怎么判断它是否合理

\(b_{r2}\) 不是抽象的调参旋钮，(F6) 给了它确定的量级和符号，可以直接核对。

| 工况 | \(b_{r2}\) 的表现 | 依据 |
|---|---|---|
| 直线行驶 | 趋近 0 | \(F_{2r}\to0\) |
| 稳态左转（\(r_2>0\)） | **与 \(r_2\) 同号**，约 \(k_\beta r_2\) | (F6) 与 \(k_\beta>0\) |
| 车速加倍、曲率不变 | 约放大 4 倍 | \(b_{r2}\propto U^2\rho\) |
...
| \(P_{11}\) | `covarianceTrailerBias` | 日志中同列族 |

三者恒满足 \(\hat r_2=r_{2,\text{kin}}+\hat b_{r2}\)，可用作日志自检。

**动力学模型也输出 \(b_{r2}\)**，但含义不同：那里 \(r_2\) 是状态，
代码反推 \(\hat b_{r2}=\hat r_2-r_{2,\text{kin}}\) 作为**诊断量**，
衡量"动力学解偏离纯滚动多远"。因此两个模型的 `ekf_b_r2` 列可以直接对比：
若运动学估出的 \(b_{r2}\) 明显小于动力学的，就是 4.6 节说的跟不上。

#### 4.5.3 整定 \(q_b\)：能调到什么程度
...
的随机游走。代入 \(\sigma_b=0.063\) rad/s、\(\tau_b=0.31\) s 得
\(q_b\approx1.1\times10^{-2}\)，比默认值大得多。但**实测并不支持照此设置**
（`tools/fusion_probe.cpp` 的扫描，其余条件同 13.1 节）：
...
| \(1.1\times10^{-2}\)（(F7a) 值） | 0.550° | 1.485°/s |
...
由 4.5.1，\(b_{r2}\) 的典型量级就是几度每秒，所以这个初值表示
"开机时对它几乎一无所知"，是合理的弱先验。
调大不会明显加快收敛（\(b_{r2}\) 的收敛由 (F11a) 的 \(P_{10}\) 通路决定，
不由初值决定）；调得过小会在起步阶段压制修正，应避免。
...
#### 4.8.2 为什么 \(\dot\phi\) 不能当第三个状态

给定 \((\phi,b_{r2})\) 与输入 \((r_1,U)\)，(F11) 的取值唯一确定，
没有剩余自由度。把它再加成状态，等于引入一个是其它状态确定函数的冗余坐标，
协方差在该方向上退化。
...
#### 4.8.3 雷达只测 \(\phi\)，怎么把 \(\dot\phi\) 带准

这是本模型可观性的实际工作方式。量测雅可比 \(H=[1,\;0]\) 第二列为零，
看似碰不到 \(b_{r2}\)。但卡尔曼增益是
...
\(P_{10}\) 不是凭空来的：它由 Van Loan 离散自动生成，就是 (F20) 中的
\(-\tfrac12 q_bT^{2}\) 交叉项。

> **这是 7.3 节"不能只填对角"的实际后果。** 若把 \(Q_d\) 简化成
> \(\operatorname{diag}(q_\phi T,\,q_b T)\)，则 \(P_{10}\) 恒为零，
> (F11a) 的第二个分量也恒为零 —— 雷达将**永远无法修正 \(b_{r2}\)**，
> \(\dot\phi\) 的偏差会一直累积而 \(\phi\) 看上去却很正常。
> 这种故障很隐蔽，移植时务必用 `testVanLoanMatchesAnalyticRandomWalk`
> 守住交叉项。

可观性行列式 \(\det\mathcal O_{\text{kin}}=-1\)（式 (F10)，恒定、与车速无关）
就是这条链恒久畅通的形式化保证。

#### 4.8.4 链条通了，为什么精度仍然有限

链条能把 \(b_{r2}\) 校准到"当前平均水平"，但过程模型假设
\(\dot b_{r2}=0+w_b\)（随机游走），而 4.5 节推出 \(b_{r2}\) 真实的时间常数是
\(\tau_b=0.3\text{–}0.5\) s。模型跟不上这段确定性动态，误差就落在 \(\dot\phi\) 上。

这解释了实测数据（13.1 节）为什么在两个量上差距不同：

| | 运动学 | 动力学 | 差距 | 原因 |
|---|---|---|---|---|
| \(\phi\) | 0.56° | 0.30° | 1.9× | 有雷达直接托底 |
| \(\dot\phi\) | 1.53°/s | 0.60°/s | **2.5×** | 纯靠过程模型 |

动力学模型把 \(r_2\) 建成**有自己微分方程的独立状态**（(F12) 第二行），
而不是一个随机游走偏置，所以 \(\dot\phi\) 明显更准。这正是第 5 章存在的理由。
